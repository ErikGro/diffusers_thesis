...
import diffusers
from diffusers import AutoencoderKL, DDPMScheduler, StableDiffusionPipeline, StableDiffusionInstructPix2PixPipeline, UNet2DConditionModel
from diffusers.optimization import get_scheduler
from diffusers.training_utils import EMAModel, cast_training_params, convert_state_dict_to_diffusers
...
def main():
    ...
    # unet.requires_grad_(False) # TODO: Should be equivalent to setting all params to false
    ...
    with torch.no_grad():
        ...
        new_conv_in.bias.copy_(unet.conv_in.bias) # TODO: Check if copied biases work
        new_conv_in.requires_grad_(True)
    ...
    optimizer = optimizer_cls(
        list(new_conv_in.parameters()) + list(lora_layers),
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                ihc_latents = vae.encode(batch["ihc_pixel_values"].to(weight_dtype)).latent_dist.sample()
                ihc_latents = ihc_latents * vae.config.scaling_factor
                ...
                noisy_latents = noise_scheduler.add_noise(ihc_latents, noise, timesteps)
                ...
                if args.conditioning_dropout_prob is not None:
                    random_p = torch.rand(bsz, device=ihc_latents.device, generator=generator)
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    target = noise_scheduler.get_velocity(ihc_latents, noise, timesteps)
                else:
