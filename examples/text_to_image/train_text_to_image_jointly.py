...
def main():
    ...
    def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch):
        ...
        with torch.autocast(accelerator.device.type):
            ...
            he_images = []
            for i in range(0, 4):
                he_image = pipeline(args.he_generation_prompt, num_inference_steps=20, generator=torch.Generator("cuda").manual_seed(i)).images[0]
                he_images.append(he_image)
                
                ihc_image = pipeline(args.translation_prompt, num_inference_steps=20, generator=torch.Generator("cuda").manual_seed(i)).images[0]
                translated_images.append(ihc_image)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            generate_he = True # torch.randint(0,2,(1,)).item() == 0

            with accelerator.accumulate(unet):
                # Convert images to latent space
                latents = vae.encode(batch["he_pixel_values" if generate_he else "ihc_pixel_values"].to(weight_dtype)).latent_dist.sample()
                ...
                encoder_hidden_states = he_prompt if generate_he else translation_prompt
